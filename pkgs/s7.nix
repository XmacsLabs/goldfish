...
stdenv.mkDerivation (_finalAttrs: {
  ...
  buildPhase = ''
    ...
    ${lib.optionalString (!static && hostPlatform.isLinux) ''
      $CC s7.o -o libs7.so \
        -shared -fPIC
    ''}
    ${lib.optionalString (!static && hostPlatform.isMinGW) ''
      $CC s7.o -o libs7.dll \
        -shared
    ''}
    ...
    ${lib.optionalString (hostPlatform.isLinux && !hostPlatform.isMusl) ''
      ./s7-repl libc.scm
    ''}
  ...
  installPhase = ''
    ...
    ''}
    ${lib.optionalString hostPlatform.isMinGW ''
