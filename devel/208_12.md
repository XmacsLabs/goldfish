...
## 2025-10-20 实现 utf16le->utf8

### What
实现 `utf16le->utf8` 函数，用于将 UTF-16LE 编码的字节向量转换为 UTF-8 编码的字节向量。

1. 使用 `bytevector-utf16le-advance` 遍历 UTF-16LE 字节向量
2. 在每次循环中：
...
### Why
需要提供 UTF-16LE 到 UTF-8 的编码转换功能，这是 Unicode 编码转换功能的重要组成部分，与现有的 `utf8->utf16le` 函数形成对称设计。

### How
实现思路：
- 利用现有的 `bytevector-utf16le-advance` 函数来遍历 UTF-16LE 字节向量
- 在遍历过程中，通过 `utf16le->codepoint` 和 `codepoint->utf8` 进行编码转换
- 使用字节向量拼接操作来构建最终的 UTF-8 结果
- 这种方法可以正确处理基本多文种平面字符和辅助平面字符（代理对）
