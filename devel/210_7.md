...
### Why
这些集合运算用于组合、筛选或比较多个 set，是 set 操作的核心能力。结果集合应保留“首次出现”的元素来源，并遵循比较器等价规则。

### How
1. set-union 按顺序合并集合，仅在结果中不存在时才加入，确保元素来源正确
2. set-intersection 只保留同时出现在所有集合中的元素（来自第一个集合）
3. set-difference 返回第一个集合与其余集合的非对称差集
4. set-xor 返回两个集合的对称差集
5. 在测试中覆盖多集合、比较器一致性与类型错误场景
