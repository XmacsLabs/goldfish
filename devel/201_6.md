...
### 2025/08/07 ✅ 新增 string-copy 的文档和测试
新增 string-copy 的完整测试文档和测试用例，支持基本的字符串拷贝功能，包括以下几种形式的调用：
- (string-copy str) - 创建整个字符串的深拷贝
- (string-copy str start) - 从start位置开始创建子串拷贝  
- (string-copy str start end) - 创建start到end范围内的子串拷贝

确保正确处理Unicode字符、边界情况、错误处理验证，以及深拷贝验证（通过eq?返回#f验证）。
