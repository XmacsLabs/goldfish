...
### 2025/08/07 ✅ ✅ 完成 string-copy 的文档和测试验证
已完成 string-copy 的完整测试用例和文档说明，支持基本的字符串深拷贝功能：
- (string-copy str) - 创建完整字符串的深拷贝
- (string-copy str start) - 从指定位置开始的子串拷贝
- (string-copy str start end) - 指定范围的子串拷贝

验证通过以下内容：
✅ 基本功能验证：空字符串、普通字符串、Unicode字符、emoji等测试
✅ 深拷贝验证：通过 equal? 和 not eq? 确保是独立对象
✅ 边界情况：start/end参数的各种边界值处理
✅ 错误处理：类型错误和范围越界检查
✅ 测试结果：539个测试用例全部通过（0失败）
